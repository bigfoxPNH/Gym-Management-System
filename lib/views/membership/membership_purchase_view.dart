...
class MembershipPurchaseView extends GetView<MembershipPurchaseController> {
  ...
  void _showPurchaseHistory() {
    ...
    // Always load fresh user purchases when opening history
    controller.loadUserPurchases(userId);
                ...
                if (controller.userPurchases.isEmpty) {
                  return Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        'Bạn chưa mua thẻ tập nào',
                        style: TextStyle(fontSize: 18, color: Colors.grey[600]),
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Mua thẻ tập đầu tiên để bắt đầu tập luyện!',
                        style: TextStyle(color: Colors.grey[500]),
                      ),
                      const SizedBox(height: 24),
                      ElevatedButton(
                        onPressed: () => Get.back(),
                        child: const Text('Mua thẻ ngay'),
                      ),
                    ],
                  );
                }
  ...
  Widget _buildPurchaseHistoryCard(MembershipPurchase purchase) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Mua ngày: ${DateFormat('dd/MM/yyyy').format(purchase.purchaseDate)}',
              style: TextStyle(color: Colors.grey[600]),
            ),

            Text(
              'Từ ${DateFormat('dd/MM/yyyy').format(purchase.startDate)} '
              'đến ${DateFormat('dd/MM/yyyy').format(purchase.endDate)}',
              style: TextStyle(color: Colors.grey[600]),
            ),
            ...
            Row(
              ...
              children: [
                Text(
                  '${NumberFormat('#,###', 'vi_VN').format(purchase.price)} VNĐ',
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.green,
                  ),
                ...
                if (purchase.status == PurchaseStatus.active &&
                  ...
                  Text(
                    'Còn ${purchase.endDate.difference(DateTime.now()).inDays + 1} ngày',
                    style: const TextStyle(
                      color: Colors.blue,
  ...
  Widget _buildStatusChip(PurchaseStatus status) {
    Color color;
    switch (status) {
