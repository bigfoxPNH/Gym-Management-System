...
class CheckoutView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Initialize checkout
    controller.initializeCheckout(membershipCard, purchaseId);
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        return _buildCheckoutForm(membershipCard);
      }),
    );
  }

  Widget _buildCheckoutForm(MembershipCard membershipCard) {
      ...
      () => Container(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(
                color: controller.selectedPaymentMethod.value?.id == method.id
                    ? Get.theme.primaryColor
                    : Colors.grey[300]!,
                width: controller.selectedPaymentMethod.value?.id == method.id
                    ? 2
                    : 1,
              ),
              borderRadius: BorderRadius.circular(8),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Obx(() {
              final fee = controller.getTransactionFee();
              if (fee > 0) {
                return Column(
                  children: [
        ...
        child: Column(
          ...
          children: [
            ...
            if (transaction.paymentMethod == PaymentMethodType.momo) ...[
              ...
              _buildCashPaymentInstructions(transaction),
            ],
            const SizedBox(height: 30),
            _buildPaymentActions(transaction),
          ],
        ),
      );
    });
            ...
            if (isDemo)
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () => controller.simulatePaymentSuccess(),
  ...
  Widget _buildCashPaymentInstructions(PaymentTransaction transaction) {
    return Card(
      elevation: 2,
      child: Padding(
        padding: const EdgeInsets.all(16),
  ...
  Widget _buildPaymentActions(PaymentTransaction transaction) {
    return Column(
      children: [
