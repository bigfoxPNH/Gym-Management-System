...
class MoMoPaymentView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final purchaseId = displayTransaction.membershipPurchaseId ?? displayTransaction.id;
      controller.initializePayment(displayCard, purchaseId);
    });
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildQrOrStatus(controller),
          const SizedBox(height: 20),
          _buildConditionalInfo(controller),
          const SizedBox(height: 20),
          _buildInstructions(),
        ],
      ),
  ...
  // Hiển thị DUY NHẤT 1 QR nếu đã có dữ liệu; nếu chưa thì hiển thị trạng thái tạo QR
  Widget _buildQrOrStatus(MoMoPaymentController controller) {
    return Obx(() {
      final qrUrl = controller.qrCodeUrl.value;
      if (qrUrl.isNotEmpty) {
        return Card(
          elevation: 2,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                // Dùng Image.network cho data URL (Flutter hỗ trợ từ 3.13+); nếu cần có thể chuyển sang Image.memory
                Image.network(qrUrl),
      ...
      // Chưa có QR → hiển thị trạng thái tạo QR
      return Card(
        elevation: 2,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        child: Padding(
          ...
          child: Column(
            children: const [
              ...
              SizedBox(height: 6),
              Text(
                'Vui lòng quét mã QR bằng MoMo để thanh toán',
  ...
  // Chỉ hiện thông tin kết quả hoặc lỗi; ẩn khi đang hiển thị QR để tránh rối
  Widget _buildConditionalInfo(MoMoPaymentController controller) {
    return Obx(() {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInstructionStep(
              '1',
              'Mở ứng dụng MoMo, chọn Quét mã QR',
            ),
            _buildInstructionStep(
              '2',
              'Quét mã QR hiển thị ở trên trong vòng 2 phút',
            ),
            _buildInstructionStep(
              '3',
