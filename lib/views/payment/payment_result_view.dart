...
class _PaymentResultViewState extends State<PaymentResultView> {
  ...
  Future<void> _updatePaymentStatus(String orderId, Map<String, String> params) async {
    try {
      ...
      await paymentService.updatePaymentStatus(orderId, PaymentStatus.completed);
      print('Payment status updated directly in database');

      // 3. Trigger CheckoutController để refresh UI nếu đang mở
      try {
        final checkoutController = Get.find<CheckoutController>();
        await checkoutController.checkPaymentStatus();
        print('CheckoutController refreshed successfully');
      } catch (e) {
        print('CheckoutController not found or error: $e');
        // Không sao vì user có thể không còn ở trang checkout
      }
