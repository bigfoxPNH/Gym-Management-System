...
class ExerciseService {
  ...
  Future<void> updateExercise(String exerciseId, Exercise exercise) async {
    try {
      final updateData = exercise.copyWith(
  ...
  Future<List<Exercise>> getExercisesByType(ExerciseType type) async {
    try {
      final snapshot = await exercisesCollection
          .where('loaiBaiTap', isEqualTo: type.name)
          .get();
  ...
  Future<Map<String, int>> getExerciseStats() async {
    try {
      ...
      final stats = <String, int>{
        ...
        'beginner': exercises.where((e) => e.doKho == ExerciseLevel.beginner).length,
        'intermediate': exercises.where((e) => e.doKho == ExerciseLevel.intermediate).length,
        'advanced': exercises.where((e) => e.doKho == ExerciseLevel.advanced).length,
        'push': exercises.where((e) => e.loaiBaiTap == ExerciseType.push).length,
        'pull': exercises.where((e) => e.loaiBaiTap == ExerciseType.pull).length,
        'compound': exercises.where((e) => e.loaiBaiTap == ExerciseType.compound).length,
        'isolation': exercises.where((e) => e.loaiBaiTap == ExerciseType.isolation).length,
        'cardio': exercises.where((e) => e.loaiBaiTap == ExerciseType.cardio).length,
        'flexibility': exercises.where((e) => e.loaiBaiTap == ExerciseType.flexibility).length,
      };
